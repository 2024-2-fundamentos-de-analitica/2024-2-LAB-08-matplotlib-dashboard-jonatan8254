...
def plot_customer_rating(df):
    plt.figure(figsize=(6, 4))
    # Se agrupa por modo de envío y se calculan estadísticas de Customer_rating
    stats = df.groupby("Mode_of_Shipment")["Customer_rating"].agg(["min", "mean", "max"]).sort_values("mean")
    
    # Se dibuja una barra de fondo que representa el rango (min a max)
    plt.barh(
        y=stats.index,
    ...
    # Se dibuja la barra que indica la media, con color según el valor
...
def create_dashboard_html():
    # Se genera el dashboard HTML que organiza las imágenes en dos columnas
    html = """
    <!DOCTYPE html>
    <html>
      <head>
